class mamba (
  ...
  exec { "install mamba":
    command  => "export HOME='/root' && bash /tmp/mambaforge-${version}.sh -b -p '/opt/conda' -f && source '/opt/conda/etc/profile.d/conda.sh' && conda activate && source '/opt/conda/etc/profile.d/mamba.sh' && mamba init --system",
    provider => 'shell',
